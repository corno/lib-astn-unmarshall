...
export function createValueUnmarshaller<PAnnotation>/*  */(
    ...
    onError: (
        type: api.T.UnmarshallError<PAnnotation>,
        annotation: PAnnotation,
        severity: api.T.DiagnosticSeverity<PAnnotation>
    ) => void,
    ...
    function createUnexpectedArrayHandler(
        message: api.T.UnmarshallError<PAnnotation>,
        annotation: PAnnotation,
    ...
    switch (definition.type[0]) {
            ...
            return {
                ...
                object: ($e) => {
                    ...
                    if ($e.token.type[0] !== 'dictionary') {
                        onError(['object is not a dictionary', {}], $e.annotation, ['warning', null])
                    }
                    ...
                    return {
                        property: ($p) => {
                            if ($p.token.wrapping[0] !== 'quote') {
                                onError(['entry key does not have quotes', null], $p.annotation, ['warning', null])
                            }
                            foundKeysBuilder.toArray().forEach(($) => {
                                if ($ === $p.token.value) {
                                    onError(['duplicate key', null], $p.annotation, ['error', null])
                                }
                            })
        ...
        case 'list': {
            ...
            return {
                array: ($e) => {
                    if ($e.token.type[0] !== 'list') {
                        onError(['array is not a list', null], $e.annotation, ['error', null])
                    }
            ...
            return {
                ...
                taggedUnion: ($tu) => {
                    ...
                    return {
                        ...
                        missingOption: () => {
                            onError(['missing option', null], $tu.annotation, ['error', null])
                            defaultInitializeValue(
            ...
            return {
                ...
                simpleString: ($e) => {
                    ...
                    switch ($e.token.wrapping[0]) {
                        case 'none': {
                            if ($d.quoted) {
                                onError(['value should have quotes', null], $e.annotation, ['warning', null])
                            }
                            break
                        }
                        case 'quote': {
                            if (!$d.quoted) {
                                onError(['value should not have quotes', null], $e.annotation, ['warning', null])
                            }
                            break
                        }
                        case 'apostrophe': {
            ...
            return {
                array: ($e) => {
                    if ($e.token.type[0] !== 'shorthand group') {
                        if (mixedIn === null) {
                            onError(['expected a group', null], $e.annotation, ['error', null])
                            defInitializeValue()
                            return dummyHandlers.array()
                        } else {
                        ...
                        function createUnmarshallerForNextValue(): mh.IValueHandler<PAnnotation> {
                            ...
                            if (nextValue === null) {
                                return {
                                    array: ($) => {
                                        onError(['superfluous element', null], $.annotation, ['error', null])
                                        return dummyHandlers.array()
                                    },
                                    object: ($) => {
                                        onError(['superfluous element', null], $.annotation, ['error', null])
